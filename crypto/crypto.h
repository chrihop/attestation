...
err_t crypto_hash_report(in crypto_hash_context_t* ctx, unsigned char* result);
...
err_t crypto_ds_import_pem_public_key(in unsigned char* pem, in size_t pem_len,
    out struct crypto_ds_context_t* key_ctx);

err_t crypto_ds_export_pem_public_key(in struct crypto_ds_context_t* key_ctx,
    out unsigned char* pem, out size_t pem_len);

err_t crypto_ds_export_public_key(
    in struct crypto_ds_context_t* ctx, out struct crypto_ds_public_key_t* pk);

err_t crypto_ds_import_public_key(
    out struct crypto_ds_context_t* ctx, in struct crypto_ds_public_key_t* pk);

err_t crypto_ds_export_private_key(
    in struct crypto_ds_context_t* ctx, out struct crypto_ds_private_key_t* sk);

err_t crypto_ds_import_private_key(
...
bool  crypto_pki_verify(in struct crypto_pki_context_t* ctx);

err_t crypto_pki_load_signature(
    in struct crypto_pki_context_t* ctx, in char* sig_b64, in size_t sig_len);

/**
 * Key Exchange
 */
err_t crypto_dh_genkey(
 ...
 * Stream Cipher
 */
err_t crypto_sc_init(struct crypto_sc_context_t* ctx, sk_t sk, size_t sk_len);

err_t crypto_sc_encrypt(in struct crypto_sc_context_t* ctx, in message_t msg,
...
bool  crypto_sc_mac_decrypt(in struct crypto_sc_mac_context_t* ctx,
