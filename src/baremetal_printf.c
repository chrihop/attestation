...
#define DBL_MAX __builtin_huge_val()
...
static size_t
_out_rev(out_fct_type out, char* buffer, size_t idx, size_t maxlen,
...
static size_t
_ntoa_format(out_fct_type out, char* buffer, size_t idx, size_t maxlen,
    char* buf, size_t len, int negative, unsigned int base, unsigned int prec,
...
static size_t
_ntoa_long(out_fct_type out, char* buffer, size_t idx, size_t maxlen,
    unsigned long value, int negative, unsigned long base, unsigned int prec,
    unsigned int width, unsigned int flags)
{
...
static size_t
_ntoa_long_long(out_fct_type out, char* buffer, size_t idx, size_t maxlen,
...
static size_t
_ftoa(out_fct_type out, char* buffer, size_t idx, size_t maxlen, double value,
    unsigned int prec, unsigned int width, unsigned int flags)
{
...
static size_t
_etoa(out_fct_type out, char* buffer, size_t idx, size_t maxlen, double value,
    unsigned int prec, unsigned int width, unsigned int flags)
{
    ...
    if (minwidth)
    {
        ...
        idx = _ntoa_long(out, buffer, idx, maxlen,
            (expval < 0) ? -expval : expval, expval < 0, 10, 0, minwidth - 1,
...
// internal vsnprintf
static int
_vsnprintf(out_fct_type out, char* buffer, const size_t maxlen,
    ...
    while (*format)
    {
        ...
        case 'b':
        {
            ...
            else
            {
...
#if defined(PRINTF_SUPPORT_LONG_LONG)
                    idx = _ntoa_long_long(out, buffer, idx, maxlen,
                        va_arg(va, unsigned long long), 0, base, precision,
                        width, flags);
#endif
                }
                else if (flags & FLAGS_LONG)
                {
                    idx = _ntoa_long(out, buffer, idx, maxlen,
                        va_arg(va, unsigned long), 0, base, precision,
                        width, flags);
                }
                else
                {
                    ...
                    idx = _ntoa_long(out, buffer, idx, maxlen, value, 0,
        ...
        case 'p':
        {
            ...
            {
                idx = _ntoa_long_long(out, buffer, idx, maxlen,
                    (uintptr_t)va_arg(va, void*), 0, 16U, precision, width,
                    flags);
            }
            else
            {
...
int
static_sprintf(char* buffer, const char* format, ...)
{
    va_list va;
    va_start(va, format);
...
int
static_snprintf(char* buffer, size_t count, const char* format, ...)
{
    va_list va;
    va_start(va, format);
...
int
static_vsnprintf(char* buffer, size_t count, const char* format, va_list va)
{
